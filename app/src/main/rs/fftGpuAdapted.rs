...
void CalculateAngles(float *v_out, uint32_t x) {
    ...
    for(uint16_t y = 0;y < uint32_t_SampleLength;y++){//Y == angle
        write2DArray(x,y,Angles,AnglesLength,sin(y * PointsDistance));
    }
}
...
void fft(float *v_out, uint32_t x) {
    float x_some = 0;
    for (uint32_t y = 0; y < uint32_t_SampleLength; y++) {//y == radius
